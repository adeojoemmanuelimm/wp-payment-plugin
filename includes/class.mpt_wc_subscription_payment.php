...
class MPT_WC_Payment_Gateway_Subscriptions extends MPT_WC_Payment_Gateway
{
    ...
    public function scheduled_subscription_payment($amount_to_charge, $renewal_order)
    {
        ...
        if (is_wp_error($response)) {
            $renewal_order->update_status('failed', sprintf('Payment Transaction Failed: (%s)', $response->get_error_message()));
    ...
    public function process_subscription_payment($order = '', $amount = 0)
    {
        ...
        $auth_code = get_post_meta($order_id, '_payment_wc_token', true);
        ...
        if ($auth_code) {
            ...
            // tokenize url
            $tokenized_url = $this->base_url . '/mptv3-pug/getpaidx/api/tokenized/charge';
            ...
            if (!is_wp_error($request) && 200 == wp_remote_retrieve_response_code($request)) {
                ...
                if ('success' === $status && in_array($response_code, $valid_response_code)) {
                    ...
                    $payment_ref = $response->data->mptRef;
                    $amount_charged = $response->data->charged_amount;

                    $order->payment_complete($order_id);
                    $message = (sprintf('Payment via Payment successful (<strong>Transaction Reference:</strong> %s | <strong>Payment Reference:</strong> %s)', $txn_ref, $payment_ref));
                    $order->add_order_note($message);
                    return true;
                }
                else {

                    return new WP_Error('payment_error', 'Payment payment failed. ' . $response->message);

                }
            }
        }

        return new WP_Error('payment_error', 'This subscription can\'t be renewed automatically. The customer will have to login to his account to renew his subscription');
    }
