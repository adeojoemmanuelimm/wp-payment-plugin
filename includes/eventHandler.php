...
require(MPT_WC_DIR_PATH . 'mypatricia-payment-php-sdk/lib/paymentEventHandlerInterface.php');
use Patricia\Payment\EventHandlerInterface;

// This is where you set how you want to handle the transaction at different stages
class PaymentEventHandler implements EventHandlerInterface
{
    private $order;
    ...
    function onInit($initializationData)
    {
        ...
        $this->order->add_order_note('Payment initialized via Payment');
        update_post_meta($this->order->get_id(), '_mpt_payment_txn_ref', $initializationData['txref']);
    ...
    function onSuccessful($transactionData)
    {
        ...
        if ($transactionData->chargecode === '00' || $transactionData->chargecode === '0') {
            if ($transactionData->currency == $this->order->get_currency() && $transactionData->amount == $this->order->get_total()) {
                ...
                $mpt_settings = get_option('woocommerce_' . $payment_method . '_settings');

                if (isset($mpt_settings['autocomplete_order']) && 'yes' === $mpt_settings['autocomplete_order']) {
                    $this->order->update_status('completed');
                }
                $this->order->add_order_note('Payment was successful on Payment');
                $this->order->add_order_note('Flutterwave transaction reference: ' . $transactionData->mptref);
            ...
            MPT_WC_Payment_Gateway::save_card_details($transactionData, $this->order->get_user_id(), $order_id);
    ...
    function onFailure($transactionData)
    {
        ...
        $this->order->add_order_note('The payment failed on Payment');
    ...
    function onRequery($transactionReference)
    {
        ...
        $this->order->add_order_note('Confirming payment on Payment');
    }
    ...
    {
        // Do something, anything!
        $this->order->add_order_note('An error occured while confirming payment on Payment');
        $this->order->update_status('on-hold');
        $customer_note = 'Thank you for your order.<br>';
    ...
    function onCancel($transactionReference)
    {
        ...
        $this->order->add_order_note('The customer clicked on the cancel button on Payment');
    ...
    function onTimeout($transactionReference, $data)
    {
        ...
        $this->order->add_order_note('The payment didn\'t return a valid response. It could have timed out or abandoned by the customer on Payment');
        $this->order->update_status('on-hold');
        $customer_note = 'Thank you for your order.<br>';
        $customer_note .= 'We had an issue confirming your payment, but we have put your order <strong>on-hold</strong>. ';
        $customer_note .= 'Please, contact us for information regarding this order.';
        $admin_note = 'Attention: New order has been placed on hold because we could not get a definite response from the payment gateway. Kindly contact the Payment support team at hi@flutterwave.com to confirm the payment. <br>';
